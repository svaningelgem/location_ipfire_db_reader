...
def fake_download_session(mocker: MockerFixture) -> MockType:
    ...
    session.head.return_value = mocker.Mock(headers={"Last-Modified": "Wed, 15 Nov 2023 19:51:32 GMT", "Content-Length": str(len(xz_compressed_bumba_word))})
...
def test_dont_redownload_db(locdb: LocationDatabase, locdb_path: Path, mocker: MockerFixture) -> None:
    ...
    LocationDatabase(locdb_path).header
